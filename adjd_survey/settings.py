"""
Django settings for adjd_survey project.
...
MIDDLEWARE = [
    ...
    'adjd_survey.middleware.brute_force_protection.BruteForceProtectionMiddleware',
    ...
    'adjd_survey.middleware.emirates_timezone.EmiratesTimezoneMiddleware',
    'django.middleware.clickjacking.XFrameOptionsMiddleware',
]

ROOT_URLCONF = 'adjd_survey.urls'
...
WSGI_APPLICATION = 'adjd_survey.wsgi.application'
...
# IMPORTANT: Set these values in .env file or environment variables
...
CORS_ALLOWED_ORIGINS = [origin.strip() for origin in os.getenv('CORS_ALLOWED_ORIGINS', 'http://localhost:5173,http://127.0.0.1:5173,https://localhost:5173,https://127.0.0.1:5173').split(',') if origin.strip()]
...
# ASGI_APPLICATION = 'adjd_survey.asgi.application'
...
# Exclude specific URLs from CSP (Azure AD login endpoint)
CSP_EXCLUDE_URL_PREFIXES = ('/api/auth/azure-login',)
...
if not DEBUG:
    ...
    # Development settings - more permissive for local development
    CSP_SCRIPT_SRC = ("'self'", "'unsafe-inline'", "'unsafe-eval'")  # Allow for development
    # Allow connections to localhost for development (both HTTP and HTTPS)
    # Also allow Azure AD and general HTTPS for API calls from HTTPS frontend
    CSP_CONNECT_SRC = (
        "'self'", 
        "http://localhost:*", 
        "https://localhost:*",
        ...
        "http:",  # Allow HTTP connections in development for mixed content
    )
...
# ===================================================================
# EMAIL CONFIGURATION (Email Communication System)
# ===================================================================

# Allow overriding the email backend from environment. If not set, default to
# console backend in DEBUG and SMTP backend in production.
EMAIL_BACKEND = os.getenv('EMAIL_BACKEND')
if not EMAIL_BACKEND:
    EMAIL_BACKEND = 'django.core.mail.backends.console.EmailBackend' if DEBUG else 'django.core.mail.backends.smtp.EmailBackend'

# If the selected backend is SMTP (explicitly or inferred), read SMTP settings
